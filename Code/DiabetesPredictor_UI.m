function DiabetesPredictorUI
    ...
    genderDropdown = uicontrol(panel1, 'Style','popupmenu','String',{'Select','Male','Female'},...
    ...
    pregnancyLabel = uicontrol(panel2, 'Style','text','String','Pregnancies:',...
                                'Position',[40 250 200 25],'HorizontalAlignment','left','Visible','off');
    pregnancyInput = uicontrol(panel2, 'Style','edit','Position',[250 250 120 25],'Visible','off');
    ...
    resultText = uicontrol(panel2, 'Style','text','String','',...
                           'Position',[50 80 350 40],'FontSize',12,'FontWeight','bold','HorizontalAlignment','center');
    ...
    uicontrol(panel3, 'Style','text','String','Any Family History of Diabetes?',...
              'Position',[60 460 350 30],'FontSize',12,'FontWeight','bold','HorizontalAlignment','center');
    ...
    responseText = uicontrol(panel3, 'Style','text','String','',...
              'Position',[40 260 370 60],'FontSize',11,'HorizontalAlignment','center');
    ...
    %% Restart
    function restartApp()
    set(familyDropdown, 'Value', 1);         % Reset dropdown
    set(responseText, 'String', '');         % Clear any previous message
    close(f);
    DiabetesPredictorUI();                   % Restart entire GUI
    end
    ...
    function predictCallback(~,~)
        ...
        for j = 1:5
            ...
            elseif isnan(val) || val < 0
                set(resultText, 'String', 'Please enter all valid inputs.', 'ForegroundColor', [1 0 0]);
                return;
            end
            userInput(j) = val;
        end
        ...
        % High glucose
        if userInput(1) > 200
            set(resultText, 'String', 'High glucose levels detected! Likely diabetic.', 'ForegroundColor', [1 0 0]);
            showPage3 = false;
            drawnow;
            pause(2);
            return;
        ...
        if strcmp(gender, 'Female')
            ...
            if isnan(preg) || preg < 0
                set(resultText, 'String', 'Enter valid number of pregnancies.','ForegroundColor',[1 0 0]);
                return;
            end
        else
        ...
        finalInput = [preg, userInput(1:2), avg_skin, userInput(3), userInput(6), avg_dpf, ageVal];
        inputNorm = (finalInput - mu) ./ sigma;
        pred = predict(model, inputNorm);

        if pred == 1
            set(resultText, 'String', 'The patient is likely diabetic.','ForegroundColor',[1 0 0]);
            showPage3 = false;
        else
            set(resultText, 'String', 'The patient is likely not diabetic.','ForegroundColor',[0 0.5 0]);
        end
    ...
    function handleFamilyHistory(~,~)
        ...
        elseif sel == 2
            set(responseText, 'String', ...
                ...
                'ForegroundColor', [1 0 0]);  % Red
        else
            set(responseText, 'String', ...
                'Great! You are not diabetic, and with no family history, you are at low risk.', ...
                'ForegroundColor', [0 0.5 0]);  % Green
        end
    end
end